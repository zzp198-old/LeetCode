...
func (this *MKAverage) AddElement(num int) {
	if len(this.arr) == this.m {
		...
		this.sum -= num
		this.delA[num]++
		if num <= this.maxA[0] { // 小于最小k个数的最大值，则要从最小k队列中删除这个数
			this.minS -= num
			this.minD++
		}
		this.delB[num]++
		if num >= this.minB[0] { // 大于最大k个数的最大值，则要从最大k队列中删除这个数
			this.maxS -= num
			this.maxD++
		}
	}
	...
	this.sum += num
	if this.maxA.Len() == 0 || num <= this.maxA[0] { // 新增数字小于等于最小k个数的最大数，入最小k队列
		heap.Push(&this.maxA, num)
		this.minS += num
	} else {
		heap.Push(&this.minA, num)
	}
	if this.minB.Len() == 0 || num >= this.minB[0] { // 新增数字大于等于最大k个数的最大数，入最大k队列
...
func (this *MKAverage) balanceHeapA() {
	for {
		for this.maxA.Len() > 0 && this.delA[this.maxA[0]] > 0 { // 删除最小k中已标记未删除的数
		...
		if this.maxA.Len()-this.minD < this.k && this.minA.Len() > 0 { // 平衡最小k和最大m-k
...
func (this *MKAverage) balanceHeapB() {
	for {
		for this.minB.Len() > 0 && this.delB[this.minB[0]] > 0 { // 删除最大k中已标记未删除的数
		...
		if this.minB.Len()-this.maxD < this.k && this.maxB.Len() > 0 { // 平衡最大k和最小m-k
